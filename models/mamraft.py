...
class mamraft(nn.Module):
    def __init__(self):
        super(mamraft, self).__init__()
        ...
        self.alternate_corr = False  # use standard or alternate correlation
        block = BasicBlock
        input_lidar = 1
        layers = [2, 2, 2, 2]
        ...
        if self.small:
            ...
            self.fnet1 = ResnetEncoder(num_layers=self.res_num, pretrained=True, num_input_images=1)  # For RGB
            self.cnet = ResnetEncoder(num_layers=self.res_num, pretrained=True, num_input_images=1)  # For context
            self.update_block = BasicUpdateBlock(hidden_dim=hdim)

        # Lidar feature extractor (ResNet-style)
        self.inplanes = 64
        ...
        # 1x1 convolutions for feature fusion
        self.conv_l4_1x1 = nn.Conv2d(512, 256, kernel_size=1)
        self.conv_l3_1x1 = nn.Conv2d(256, 256, kernel_size=1)
        self.conv_l2_1x1 = nn.Conv2d(128, 256, kernel_size=1)
        ...
        # Vision Mamba temporal encoder (5-frame input)
        self.vmamba = VisionMamba(
            img_size=(32, 640),
            patch_size=(16, 16),
            embed_dim=192,
            depth=24,
    ...
    def forward(self, image1, image2, iters=10, flow_init=None, upsample=True, test_mode=False):
        """
        Estimate relative pose (rotation) between image1 (RGB) and image2 (LiDAR projection)
        using correlation and temporal reasoning via Video Mamba.
        """
        ...
        fmap1 = self.fnet1(image1)  # shape: (B, C, H/8, W/8)

        # Extract hierarchical features from LiDAR input
        ...
        corr_fn = AlternateCorrBlock(fmap1, l2_up, radius=self.corr_radius) if self.alternate_corr \
                else CorrBlock(fmap1, l2_up, radius=self.corr_radius)

        # Run context encoder on image1 to produce hidden and input states
        ...
        # Initialize coordinate grids for optical flow
        coords0, coords1 = self.initialize_flow(image1)
        if flow_init is not None:
            coords1 = coords1 + flow_init

        # Iterative refinement using update block
        x_list = []
        for itr in range(iters):
            coords1 = coords1.detach()
            corr = corr_fn(coords1)  # lookup correlation
        ...
        # Stack all flow estimates along temporal dimension
        x_tensor = torch.cat(x_list, dim=3)  # [B, 2, H, W*T]

        # Prepare input for Vision Mamba
        B, C, H, W = x_tensor.shape
        T = 5
        batch = B // T
        assert B % T == 0, "Batch size must be divisible by number of frames (T=5)"
        x_tensor = x_tensor.view(batch, T, C, H, W)
        x_tensor = x_tensor.permute(0, 2, 1, 3, 4).contiguous()

        # Temporal reasoning with Vision Mamba
        x_tensor = self.vmamba(x_tensor)  # shape: (batch, 2048)
        ...
        rot = rot.repeat_interleave(T, dim=0)  # duplicate for all frames
    ...
    def unfreeze_selected_layers(self):
        ...
        for param in self.vmamba.parameters():
    ...
    def unfreeze_selected_layer_all(self):
