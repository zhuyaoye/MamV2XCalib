...
class CostVolumeLayer(nn.Module):
    ...
    def forward(self, refimg_fea, targetimg_fea):
        ...
        targetimg_fea = F.unfold(targetimg_fea, (2*self.max_disp+1, 2*self.max_disp+1), padding=self.max_disp).view(b, c, 2*self.max_disp+1, 2*self.max_disp+1**2, h, w)
        cost = refimg_fea.view(b, c, h, w)[:, :, np.newaxis, np.newaxis] * targetimg_fea.view(b, c, 2*self.max_disp+1, 2*self.max_disp+1**2, h, w)
        cost = cost.sum(1)
...
class LCCNet(nn.Module):
    ...
    def __init__(self, image_size, use_feat_from=1, md=4, use_reflectance=False, dropout=0.0,
        ...
        #self.corr= CostVolumeLayer(max_disp=md)
