...
import mathutils
import sys
#sys.path.append('/home/JJ_Group/zhuyy2402/MambaV2XCalib/models')
...
def main(_config, seed):
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            for idx2 in range(len(sample['rgb'])):
                sample['point_cloud'][idx2] = sample['point_cloud'][idx2].cuda()
            pc_lidar = sample['point_cloud'][len(sample['rgb'])-1].clone()
            for idx2 in range(len(sample['rgb'])-1):
                pc_lidar2 = sample['point_cloud'][idx2].clone()
                pc_lidar = torch.cat([pc_lidar, pc_lidar2], dim=1)
        ...
        with torch.no_grad():
            ...
            for iteration in range(start, len(weights)):
                ...
                if _config['iterative_method'] in ['single_range', 'single']:
                    ...
                    R_predicted = models[iteration](rgb_resize, lidar_resize)[-1]
