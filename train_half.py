...
from DatasetLidarCamera import DatasetI2V_DAIR_V2X_Camera, DatasetI2V_tumtraf
from losses import listLoss
...
def train(model, optimizer, rgb_img, refl_img, target_transl, target_rot, loss_fn, point_clouds, loss):
    ...
    rot_err11=rot_err[11]
    
    return losses, rot_err11
...
def val(model, rgb_img, refl_img, target_transl, target_rot, loss_fn, point_clouds, loss):
    ...
    # Run model
    with torch.no_grad():
        rot_errlist = model(rgb_img, refl_img,test_mode=True,iters=12)

    if loss == 'points_distance' or loss == 'combined':
        losses = loss_fn(point_clouds, target_transl, target_rot, rot_errlist)
    elif loss=='listloss':
        losses = loss_fn(point_clouds, target_transl, target_rot, rot_errlist)
    else:
        losses = loss_fn(target_transl, target_rot, rot_errlist)

   
    rot_err=rot_errlist[0]
...
def main(_config, _run, seed):
    ...
    max_t=_config['max_t']
    max_r=_config['max_r']
    if _config['dataset']=='v2x_seq':
        dataset_train = DatasetI2V_DAIR_V2X_Camera(path='/home/JJ_Group/zhuyy2402/FFNet-VIC3D/data/v2x-seq/V2X-Seq/cooperative-vehicle-infrastructure', 
                                    ...
                                    sensortype="lidar")
        dataset_val = DatasetI2V_DAIR_V2X_Camera(path='/home/JJ_Group/zhuyy2402/FFNet-VIC3D/data/v2x-seq/V2X-Seq/cooperative-vehicle-infrastructure', 
                                    split_path='/home/JJ_Group/zhuyy2402/DAIR-V2X/data/split_datas/cooperative-split-data-spd.json',
                                    max_t=max_t, max_r=max_r, split='val',
                                    sensortype="lidar")
    elif _config['dataset']=='tumtraf':
        dataset_train = DatasetI2V_tumtraf(path='/home/JJ_Group/zhuyy2402/tumtraf_v2x_cooperative_perception_dataset/train',
                                  max_t=max_t, max_r=max_r, split='train')
        dataset_val = DatasetI2V_tumtraf(path='/home/JJ_Group/zhuyy2402/tumtraf_v2x_cooperative_perception_dataset/val',
                                  max_t=max_t, max_r=max_r, split='val')
    else:
    ...
    if _config['loss'] == 'listloss':
        loss_fn = listLoss(_config['rescale_transl'], _config['rescale_rot'], _config['weight_point_cloud'])
    else:
    ...
    if _config['optimizer'] == 'adam':
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[90,160], gamma=0.24)
    else:
